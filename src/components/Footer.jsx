...
export default function Footer() {
  return (
    <footer className="border-t border-blue-500/8 mt-24" style={{ background: 'rgba(6, 10, 22, 0.95)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="md:col-span-1">
            <Link to="/" className="flex items-center gap-2.5 mb-5">
              ...
              <div className="flex flex-col leading-none">
                ...
                <span className="text-[10px] text-blue-500/60 font-medium tracking-widest uppercase mt-0.5">AI Tool Library</span>
              </div>
            </Link>
            ...
            <div className="flex gap-2.5">
              {[Twitter, Github, Linkedin].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="w-9 h-9 rounded-lg border border-blue-500/15 flex items-center justify-center text-white/30 hover:text-blue-400 hover:border-blue-500/40 transition-all duration-200"
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={item.label}>
                  <Link to={item.path} className="text-white/35 hover:text-blue-400 text-sm transition-colors duration-200">
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['Productivity', 'Travel', 'Education', 'Automation', 'Business', 'Crypto'].map(item => (
                <li key={item}>
                  <a href="#" className="text-white/35 hover:text-blue-400 text-sm transition-colors duration-200">{item}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="border-t border-blue-500/8 mt-12 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
